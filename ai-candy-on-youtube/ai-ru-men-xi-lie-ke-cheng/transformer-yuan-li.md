...
**Decoder中使用的也是同样的结构。** 不同的地方在于，在Encoder的前端增加了一个掩码(Masked)多头自注意力机制（t时间后的信息置为0), 其输出Q。再与前面的 Encoder 的输出（K, V）, 作为输入, 进入类似一个Encoder 模块，输出，计算一遍注意力机制的得分（点积）。 之后，再进入前馈神经网络模块。
